...
import os
import sys
sys.path.append(os.path.dirname(os.path.abspath(os.path.dirname(__file__)))+'/original/yolov5')
...
#%%
PED_MODEL_PATH = './weights/car_pedestrian.pt'
CROSS_MODEL_PATH = './weights/cross.pt'

#%%
if len(sys.argv) != 2:
	raise Exception("Please enter valid target video name(ex: python TRIP.py target.mp4)")

TEST_VIDEO_PATH = './video/target'
TEST_VIDEO_NAME = sys.argv[1]
TEST_VIDEO = os.path.join(TEST_VIDEO_PATH, TEST_VIDEO_NAME)
if not os.path.isfile(TEST_VIDEO):
	raise Exception(TEST_VIDEO_NAME + " doesn't exist")
...
SAVE_VIDEO_PATH = './video/output'
SAVE_VIDEO_NAME = CURRENT_TIME + '.mp4'
...
out = cv2.VideoWriter(SAVE_VIDEO, fourcc, cap.get(cv2.CAP_PROP_FPS), (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)), int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))))
...
#%%
os.chdir(os.path.dirname(os.path.abspath(os.path.dirname(__file__)))+'/original/yolov5')
...
while cap.isOpened():
	...
	if cv2.waitKey(1) == ord('q'):
		break
print("Output saved as " + SAVE_VIDEO)
