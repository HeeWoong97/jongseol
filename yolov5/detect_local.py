...
while cap.isOpened():
  ...
  peds_dict = detect(img, ped_stride, ped_device, ped_model, ped_class_names, [], ped_colors, annotator)
  detect(img, cross_stride, cross_device, cross_model, cross_class_names, ['빨간불', '초록불'], cross_colors, annotator)
  ...
  if len(peds):
    for ped in peds:
      ...
      _in_cross = int(cy1) <= int(py2) <= int(cy2) and int(cx1) <= int(px1) and int(px2) <= int(cx2)
